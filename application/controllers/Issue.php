...
class Issue extends BaseController
{
   ...
   function index()
   {
      ...
      $data['searchTex'] = $searchText;

      $this->load->library('pagination');
      $count = $this->issue_model->issueListingCount($searchText);
      // echo ' count: ' . $count;

      $returns = $this->paginationCompress("issue/index/", $count, 10, 3);
      // echo ' segment-News: ' . $returns['segment'];

      $data['issueRecords'] = $this->Issue_model->issueListing($searchText, $returns["page"], $returns["segment"]);

      $this->global['pageTitle'] = '議題管理';

      $this->loadViews("issue/issue", $this->global, $data, NULL);
   }
