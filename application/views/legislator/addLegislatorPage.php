<?php
$yid = $yearTitle->yearid;
?>
<div class="content-wrapper">
	<section class="content-header">
		<h1>
			<i class="fa fa-users"></i> 新增 「<?php echo $yearTitle->title; ?>」 的委員列表
	...
	$repeat = $this->session->flashdata('insert-legname-repeat');
	if ($repeat) {
		?>
		<div id="alert-error" class="alert-absoulte alert alert-danger alert-dismissable">
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<?php echo $this->session->flashdata('insert-legname-repeat'); ?>
		</div>
	<?php } ?>
	<div class="col-xs-12 text-right">
		<div class="form-group">
			<a class="btn btn-warning" href="<?php echo base_url(); ?>legislator/legislatorListPage/<?php echo $yid; ?>">返回</a>
		</div>
	</div>
			...
			<div class="col-xs-12">
				<div class="box">
					<div class="box-header">
						<h3 class="box-title">從黨員列表的名單來新增委員</h3>
						<div class="box-tools">
							<form action="<?php echo base_url() ?>legislator/addLegislatorPage/<?php echo $yid; ?>" method="POST" id="searchList">
								<div class="input-group">
									<input type="text" name="searchText" value="<?php echo $searchText; ?>" class="form-control input-sm pull-right" style="width: 250px;height:30px" placeholder="搜尋黨員姓名" />
					...
					<div class="box-body table-responsive no-padding">
						<table class="table table-hover title-center">
							...
							<?php
							if (!empty($partyMemberInfo)) {
								foreach ($partyMemberInfo as $record) {
									?>
									<tr>
										...
										<td class=" text-center">
											<a class="btn btn-sm btn-success" href="<?php echo base_url() . 'legislator/insertToLegislator/'  . $yid . '/' . $record->memid; ?>" title="新增"><i class="fa fa-plus fa-lg"></i></a>
										</td>
									</tr>
								<?php
									}
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		jQuery('ul.pagination li a').click(function(e) {
			...
			jQuery("#searchList").attr("action", baseURL + "legislator/addLegislatorPage/" + yearId + '/' + value);
